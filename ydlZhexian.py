...
def YdlZhexian():
    ...
    sql = '''
          ...
          WHERE provinceName="意大利"
          '''
    # read_sql_query的两个参数: sql语句， 数据库连接
    YDL_data = pd.read_sql_query(sql, engine)

    # 输出employee表的查询结果
    Y = YDL_data['confirmedCount']
    Y_dead = YDL_data['deadCount']
    ...
    x_data = list(YDL_data['thedate'])
    # 累计确诊
    y1_data = list(YDL_data['confirmedCount'])
    # 累计治愈
    y2_data = list(YDL_data['curedCount'])
    # 累计死亡
    y3_data = list(YDL_data['deadCount'])
    ...
    line = (Line()
        ...
        .set_global_opts(
        title_opts=opts.TitleOpts(title='意大利疫情随时间变化趋势')
    ))
